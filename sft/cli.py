...
def upload(filepath, duration, service):
    ...
    try:
        ...
        click.echo(f"Uploaded {filepath.name} {format_size(result['size'])} sha256:{result['sha256'][:16]}...")
        click.echo(f"The file will be deleted in {duration} ({expiry_time.strftime('%Y-%m-%d %H:%M:%S')})")
...
def download(file_id, service, output):
    ...
    try:
        ...
        response = requests.get(
            f'{service_url}/download/{file_id}',
            stream=True,
            timeout=300
        )
        ...
        click.echo(f"File downloaded {filename} {format_size(info['size'])} sha256:{info['sha256'][:16]}...")
        click.echo(f"The file will be deleted in {time_remaining} ({expiry_time.strftime('%Y-%m-%d %H:%M:%S')})")
